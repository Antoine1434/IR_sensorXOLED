...
uint16_t offset;
first = (*(const uint8_t *)(u8x8->font+0));
last = (*(const uint8_t *)(u8x8->font+1));
tiles = (*(const uint8_t *)(u8x8->font+2));
tiles *= (*(const uint8_t *)(u8x8->font+3));
...
for( i = 0; i < 8; i++ )
{
buf[i] = (*(const uint8_t *)(u8x8->font+offset));
offset++;
}
}
else
...
void u8x8_DrawGlyph(u8x8_t *u8x8, uint8_t x, uint8_t y, uint8_t encoding)
{
uint8_t th = (*(const uint8_t *)(u8x8->font+2));
uint8_t tv = (*(const uint8_t *)(u8x8->font+3));
uint8_t xx, tile;
uint8_t buf[8];
th += x;
tv += y;
